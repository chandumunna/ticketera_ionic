...
export class PromotorTicketValidationPage {
    ...
    scanQR() {
        this.barcodeScanner.scan().then((scannerData: BarcodeScanResult) => {
            this.ticketProvider.validateQR(+scannerData.text, this.event.id).subscribe(resp => this.ticket = resp);
            this.isValid = true;
        }, (err) => {
            console.log('No se ha podido escanear el QR!');
        });
    }
    ...
    goEventStatistics() {
        this.events.publish('page:changed', 'PromotorEventStatisticsPage', { event: this.event });
    }
